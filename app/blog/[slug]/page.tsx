        ...
        <div className="min-h-screen bg-brand-dark">
            ...
            <header className="relative border-b border-slate-800 bg-slate-900/80 pt-24 pb-16 overflow-hidden">
                ...
                <div className="container relative z-10 mx-auto px-4 md:px-6 max-w-5xl">
                    <Link href="/blog" className="inline-flex items-center text-sm font-medium text-brand-primary hover:text-white mb-8 transition-colors">
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Back to Blog
                    </Link>

                    {post.categories?.nodes[0] && (
                        <Link href={`/blog/category/${post.categories.nodes[0].slug}`} className="mb-6 inline-block rounded-full bg-brand-primary/10 border border-brand-primary/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-brand-primary hover:bg-brand-primary/20 transition-colors">
                            {post.categories.nodes[0].name}
                        </Link>
                    )}

                    <h1 className="mb-6 text-3xl font-extrabold leading-tight tracking-tight text-white md:text-5xl lg:text-6xl" dangerouslySetInnerHTML={{ __html: post.title }}>
                    </h1>
